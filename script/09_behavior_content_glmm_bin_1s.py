...
Same logic as 09_behavior_content_glmm.py but using 1-second binned data instead of TR (1.5s) binned data.
...
class BehavioralContentAnalysis1s:
    def __init__(self, output_dir: Path, data_dir: Path, n_trim_seconds: int = 21,
                 coding_type: str = 'deviation', reference_group: str = 'affair'):
        """
        Initialize behavioral analysis pipeline for 1-second binned data
        ...
        n_trim_seconds : int
            Number of seconds to trim from the beginning of each sequence (21s = 14 TRs * 1.5)
        coding_type : str
        ...
        self.n_trim_seconds = n_trim_seconds
    ...
    def load_data(self):
        ...
        print("\n=== Starting data loading (1-second binned) ===")
        try:
            # Load 1-second binned behavioral data
            affair_beh_file = self.output_dir / '08_behav_results_bin_1s' / 'individual_response_evidence_affair_1s.pkl'
            paranoia_beh_file = self.output_dir / '08_behav_results_bin_1s' / 'individual_response_evidence_paranoia_1s.pkl'

            print(f"Loading 1s-binned behavioral data from:\n  {affair_beh_file}\n  {paranoia_beh_file}")

            affair_data = pickle.load(open(affair_beh_file, 'rb'))
            paranoia_data = pickle.load(open(paranoia_beh_file, 'rb'))
            print(f"Successfully loaded 1s-binned behavioral data files")

            # Extract and process binary responses (trimming first 21 seconds instead of 14 TRs)
            print("Processing binary responses...")
            self.affair_responses = np.array([resp['binary_response'][self.n_trim_seconds:]
                                        for _, resp in affair_data.iterrows()])
            self.paranoia_responses = np.array([resp['binary_response'][self.n_trim_seconds:]
            ...
            # Load 1-second binned content features
            content_file = self.data_dir / '08_stimuli_annot_bin_1s' / '10_story_annotations_1s.csv'
            print(f"\nLoading 1s-binned content features from: {content_file}")
            ...
            print(f"After trimming {self.n_trim_seconds} seconds")

            self.data_loaded = True
            print("\n=== Data loading complete (1-second binned) ===")
    ...
    def analyze_timepoint_differences(self):
        """Analyze group differences at each timepoint (1-second resolution)"""
        print("\n=== Starting timepoint difference analysis (1-second resolution) ===")
        if not self.data_loaded:
            raise RuntimeError("Data not loaded. Call load_data() first.")

        n_timepoints = self.affair_responses.shape[1]
        print(f"Analyzing {n_timepoints} timepoints (seconds)")
    ...
    def prepare_data_for_glmm(self):
        ...
        print(f"Prepared data: {n_subjects} subjects, {dv.shape[1]} timepoints (seconds)")
    ...
    def analyze_content_relationships(self):
        ...
        print("\n=== Running GLMM analysis for content relationships (1-second resolution) ===")
    ...
    def save_results(self, results):
        ...
        save_dir = self.output_dir / '09_behavior_content_glmm_bin_1s' / self.coding_type
        ...
        with open(save_dir / "behavioral_content_analysis_1s.json", 'w') as f:
        ...
        if 'content_relationships' in results and 'feature_results' in results['content_relationships']:
            ...
            if summary_data:
                ...
                summary_df.to_csv(save_dir / 'feature_summary_1s.csv', index=False)
                print(f"Saved feature summary CSV with {len(summary_data)} features")

        print("Results saved successfully")

def main():
    print("\n=== Starting behavioral content analysis (1-second resolution) ===")
    # Setup paths
    load_dotenv()
    ...
    data_dir = Path(scratch_dir) / 'output_RR'  # Changed to output_RR since 1s annotations are there

    print(f"Output directory: {output_dir}")
    print(f"Data directory: {data_dir}")

    # Initialize analysis with deviation coding
    analysis = BehavioralContentAnalysis1s(
    ...
    print("\n=== Analysis complete (1-second resolution) ===")
