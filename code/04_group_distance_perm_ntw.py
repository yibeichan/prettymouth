...
def save_results_as_npz(observed_distance_matrix, p_values, p_values_corrected, permuted_distances, distance_output_dir):
    results_path = os.path.join(distance_output_dir, "distance_per_ntw_permutated.npz")
...
if __name__ == '__main__':
    ...
    _, _, _, networknames_idx_dict = get_roi_and_network_ids(atlas_data, n_parcel)

    network_mtx_affair, index_key_map_affair = reconstruct_matrix(affair_coflt, networknames_idx_dict)
    network_mtx_paranoia, index_key_map_paranoia = reconstruct_matrix(paranoia_coflt, networknames_idx_dict)
    ...
    # save the matrices
    np.save(os.path.join(distance_output_dir, "affair_ntw_coflt.npy"), network_mtx_affair)
    np.save(os.path.join(distance_output_dir, "paranoia_ntw_coflt.npy"), network_mtx_paranoia)

    # save dictionary maps
    with open(os.path.join(distance_output_dir, "affair_ntw_index_key_map.pkl"), "wb") as f:
        pickle.dump(index_key_map_affair, f)
    with open(os.path.join(distance_output_dir, "paranoia_ntw_index_key_map.pkl"), "wb") as f:
        pickle.dump(index_key_map_paranoia, f)

    log_dir = os.path.join(output_dir, "logs")
    setup_logging(base_dir=base_dir, task="group_distance_perm_ntw", task_id=f"{n_parcel}parcel")

    print(f"Initial memory usage: {monitor_memory():.2f} GB")
    
    main(network_mtx_affair, network_mtx_paranoia, distance_output_dir)
