...
def main():
    ...
    try:
        ...
        if args.trim:
            output_dir = Path(scratch_dir) / "output" / f"{args.group_name}_hmm_{args.n_states}states_ntw_{args.res}_trimmed"
        else:
            output_dir = Path(scratch_dir) / "output" / f"{args.group_name}_hmm_{args.n_states}states_ntw_{args.res}"
...
def load_group_data(scratch_dir: str, group_name: str, res: str) -> np.ndarray:
    ...
    try:
        ...
        group_files = sorted(data_path.glob(f"{group_name}_*.npy"))
        
        if not group_files:
            raise FileNotFoundError(f"No data files found for group {group_name}")
            
        if len(group_files) != 18:
            raise ValueError(f"Expected 18 networks, found {len(group_files)}")
