...
def process_scenario_individual_rms(scenario, n_parcel, coft_output_dir, RMS_output_dir, index_dicts):
    data = read_coflt_data(n_parcel, scenario, coft_output_dir)
    num_subjects = data.shape[0]
    
    for subject_index in range(num_subjects):
        subject_data = data[subject_index, ...]
        calculate_and_save_rms(subject_data, index_dicts, f"{scenario}_subj{subject_index}", RMS_output_dir)
...
def main(n_parcel, parcellation_dir, coft_output_dir, RMS_output_dir):
    ...
    for scenario in scenarios:
        ...
        process_scenario_individual_rms(scenario, n_parcel, coft_output_dir, RMS_output_dir, index_dicts)
...
if __name__ == "__main__":
    ...
    RMS_output_dir = os.path.join(scratch_dir, "output", f"RMS_per_subject_{cal_type}", f"{n_parcel}parcel")
