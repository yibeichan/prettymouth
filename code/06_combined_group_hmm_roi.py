...
def main():
    ...
    try:
        ...
        if args.trim:
            output_dir = Path(scratch_dir) / "output" / f"{group_name}_hmm_{args.n_states}states_roi_{args.res}_trimmed"
        else:
            output_dir = Path(scratch_dir) / "output" / f"{group_name}_hmm_{args.n_states}states_roi_{args.res}"
...
def load_group_data(scratch_dir: str, res: str) -> np.ndarray:
    ...
    try:
        data_path1 = Path(scratch_dir) / "output" / f"atlas_masked_{res}" / "affair_group_data_roi.npy"
        data_path2 = Path(scratch_dir) / "output" / f"atlas_masked_{res}" / "paranoia_group_data_roi.npy"
        # Load and average data
        group_data1 = np.load(data_path1)
        group_data2 = np.load(data_path2)
