...
class SingleGroupHMM:
    ...
    def preprocess_data(self, 
        ...
        try:
            processed = []
            for subject_idx, subject in enumerate(tqdm(data, desc="Processing subjects")):
                subject_data = subject.T  # [n_timepoints, n_features]
                
                # Check for constant features
                std = np.std(subject_data, axis=0)
                if np.any(std == 0):
                    logging.warning(f"Subject {subject_idx} has constant features: {np.where(std == 0)[0]}")
                
                if normalize:
                    # Add try-except around zscore
                    try:
                        subject_data = zscore(subject_data, axis=0)
                        
                        # Check for invalid values after normalization
                        if np.any(~np.isfinite(subject_data)):
                            invalid_features = np.where(~np.isfinite(subject_data).any(axis=0))[0]
                            logging.error(f"Subject {subject_idx} has invalid values after normalization in features: {invalid_features}")
                            
                            # Optional: Handle invalid features
                            # subject_data[:, invalid_features] = 0  # or some other handling
                    except Exception as e:
                        logging.error(f"Error in z-score normalization for subject {subject_idx}: {e}")
                        raise
            ...
            # Final validation with more detailed error message
            invalid_mask = ~np.isfinite(self.processed_data)
            if np.any(invalid_mask):
                invalid_rows, invalid_cols = np.where(invalid_mask)
                n_invalid = len(invalid_rows)
                logging.error(f"Found {n_invalid} invalid values in processed data")
                logging.error(f"First few invalid positions (row, col): {list(zip(invalid_rows[:5], invalid_cols[:5]))}")
...
def main():
    ...
    parser.add_argument('--group_name', type=str, required=True,
                       help='Name identifier for the group')
    ...
    try:
        # Setup logging
        log_file = f"{args.group_name}_analysis.log"
        ...
        if args.trim:
            output_dir = Path(scratch_dir) / "output" / f"{args.group_name}_hmm_{args.n_states}states_roi_{args.res}_trimmed"
        else:
            output_dir = Path(scratch_dir) / "output" / f"{args.group_name}_hmm_{args.n_states}states_roi_{args.res}"
        ...
        group_data = load_group_data(scratch_dir, args.group_name, args.res)
...
def load_group_data(scratch_dir: str, group_name: str, res: str) -> np.ndarray:
    """Load and validate group data."""
    try:
        data_path = Path(scratch_dir) / "output" / f"atlas_masked_{res}" / f"{group_name}_group_data_roi.npy"
        # Load and average data
        group_data = np.load(data_path)
