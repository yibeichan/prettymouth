...
def calculate_distance_matrix(group1_avg, group2_avg, indices):
    n, m, _ = group1_avg.shape
    distance_matrix = np.zeros((n, m), dtype=np.float32)
    
    for i in range(n):
        for j in range(i, m):
            row_A = group1_avg[i, j, :]
            row_B = group2_avg[i, j, :]

            distances = correlation(row_A, row_B)
...
def permute_and_compute_distances(group1_tsISC, group2_tsISC, n_permutations=1000, n_jobs=4):
    ...
    print(f"all_tsISC shape: {all_tsISC.shape}")
    # all_tsISC shape (38, 400, 400, 475)
    ...
    print(f"group1_avg shape: {group1_avg.shape}")

    del group1_tsISC, group2_tsISC
    gc.collect()

    indices = np.triu_indices(n_parcel)
    print("Start to calculate observed distance matrix")
...
def save_results_as_npz(observed_distance_matrix, p_values, p_values_corrected, permuted_distances, distance_output_dir):
    results_path = os.path.join(distance_output_dir, "distance_per_parcel_permutated_pearsonr.npz")
...
def main(affair_coflt, paranoia_coflt, distance_output_dir):
    ...
    observed_distance_matrix, p_values, p_values_corrected, permuted_distances = permute_and_compute_distances(
        group1_tsISC, group2_tsISC, n_permutations=10000, n_jobs=4
    )
