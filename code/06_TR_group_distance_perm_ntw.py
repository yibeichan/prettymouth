...
from utils.coflt import read_coflt_data, reconstruct_matrix, permutation_test, fdr_correction
...
def calculate_distance_matrix(group1: np.ndarray, group2: np.ndarray, indices: Tuple[np.ndarray, np.ndarray]) -> np.ndarray:
    ...
    # Calculate Euclidean distance for each pair of nodes and each TR
    for n in range(n_TR):
        for i, j in zip(*indices):
            dist = euclidean(group1[:, i, j, n], group2[:, i, j, n])
            distance_matrix[i, j, n] = dist
            if i != j:
                distance_matrix[j, i, n] = dist
...
def main(n_parcel: int, output_dir: str, parcellation_dir: str) -> None:
    """
    ...
    _, _, _, node_idx_dict = get_roi_and_network_ids(parcellation, n_parcel)
    ...
    observed, null_distribution, p_values = permutation_test(node_mtx_affair, node_mtx_paranoia, n_permutations=10000, n_jobs=12)
    ...
    output_file = os.path.join(distance_output_dir, f"TR_group_distance_perm_ntw.npz")
...
if __name__ == "__main__":
    ...
    main(n_parcel, output_dir, parcellation_dir)